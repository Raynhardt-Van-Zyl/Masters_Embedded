...
if __name__ == '__main__':
    ...
    from params import StereoCamera
    ...
    #Camera parameters
    cam = Camera( dataset.cam.fx, dataset.cam.fy, dataset.cam.cx, dataset.cam.cy, dataset.cam.width, dataset.cam.height, params.frustum_near, params.frustum_far,  dataset.cam.baseline)
    ...
    while not rospy.is_shutdown():
        ...
        dataset.getImages()
        featurel = ImageFeature(dataset.left.image, params)
        featurer = ImageFeature(dataset.right.image, params)
        timestamp = dataset.timestamp
        print(timestamp)

        print("ImageFeature run")

        time_start = time.time()  

        t = Thread(target=featurer.extract)
        t.start()
        t2 = Thread(target=featurel.extract)
        t2.start()
        t.join()
        t2.join()
        ...
        frame = StereoFrame(count, g2o.Isometry3d(), featurel, featurer, cam, timestamp=timestamp)
